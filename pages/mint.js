...
export default function Mint() {
  ...
  // FOR MINTING
  const [how_many_punks, set_how_many_punks] = useState(1)

  const [dpunksContract, setDpunksContract] = useState(null)
  ...
  const [punkPrice, setPunkPrice] = useState(0)
...
//
  ...
  async function callContractData(wallet) {
    ...
    const dpunksContract = new window.web3.eth.Contract(ABI, ADDRESS)
    setDpunksContract(dpunksContract)
    ...
    const punkPrice = 2 
    setPunkPrice(punkPrice)
   
  }
  
  async function mintBanana(how_many_punks) {
    if (dpunksContract) {
 
      const price = Number(punkPrice)  * how_many_punks 

      const gasAmount = await dpunksContract.methods.mintBoringBanana(how_many_punks).estimateGas({from: walletAddress, value: price})
      console.log("estimated gas",gasAmount)

      console.log({from: walletAddress, value: price})

      dpunksContract.methods
            .mintBoringBanana(how_many_punks)
  ...
  return (
    <div id="bodyy" className="flex flex-col items-center justify-center min-h-screen py-2">
      <Head>
        <title>Filecoin Punks</title>
        <link rel="icon" href="/images/favicon.jpg" />

        <meta property="og:title" content="Boring Bananas Co." key="ogtitle" />
        <meta property="og:description" content="Here at Punks in Filecoin, we specialise in the world's finest digital punks. We've put together a team spanning 3 continents, to bring you some of the most ‍NUTRITIOUS and DELICIOUS
bananas out known to man." key="ogdesc" />
        <meta property="og:type" content="website" key="ogtype" />
        <meta property="og:url" content="https://boringbananas.co/" key="ogurl"/>
        <meta property="og:image" content="https://boringbananas.co/images/cycle-0.png" key="ogimage"/>
...
bananas out known to man." key="twdesc" />
        <meta name="twitter:image" content="https://boringbananas.co/images/cycle-0.png" key="twimage" />
      </Head>
        ...
        <div className="md:w-2/3 w-4/5">
          ...
          <div className="mt-6 border-b-2 py-6">
            ...
            <div className="flex flex-col items-center">
                ...
                <span className="flex Poppitandfinchsans text-5xl text-white items-center bg-grey-lighter rounded rounded-r-none my-4 ">TOTAL PUNKS MINTED:  <span className="text-blau text-6xl"> {!signedIn ?  <>-</>  :  <>{totalSupply}</> } / 10,000</span></span>
                ...
                <div id="mint" className="flex justify-around  mt-8 mx-6">
                  ...
                  <input 
                                      ...
                                      min="1"
                                      max="20"
                                      value={how_many_punks}
                                      onChange={ e => set_how_many_punks(e.target.value) }
                                      name="" 
                                      className="Poppitandfinchsans pl-4 text-4xl  inline bg-grey-lighter  py-2 font-normal rounded text-grey-darkest  font-bold"
                                  />
                  
                  <span className="flex Poppitandfinchsans text-5xl text-white items-center bg-grey-lighter rounded rounded-r-none px-3 font-bold">PUNK!</span>
    
                </div>
                {saleStarted ? 
                <button onClick={() => mintBanana(how_many_punks)} className="mt-4 Poppitandfinchsans text-4xl border-6 bg-blau  text-white hover:text-black p-2 ">MINT {how_many_punks} punk for {(punkPrice * how_many_punks)} FIL + GAS</button>        
